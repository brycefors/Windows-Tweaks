...
$registrySubPath = "Control Panel\Desktop\WindowMetrics"
$name = "MinAnimate"
$value = "0"
...
Write-Host "----------------------------------------------------------------" -ForegroundColor Cyan
Write-Host "This script disables minimize/maximize window animations"
Write-Host "by setting MinAnimate to 0."
Write-Host ""
...
# Logic
Write-Host "Applying window animation tweak to all user profiles..." -ForegroundColor White
...
try {
    Set-MinAnimateValue -HiveRoot "HKEY_USERS\.DEFAULT"
...
foreach ($entry in $profileEntries) {
    ...
    if (Test-Path $loadedHivePath) {
        try {
            Set-MinAnimateValue -HiveRoot "HKEY_USERS\$sid"
    ...
    try {
        Set-MinAnimateValue -HiveRoot $tempHiveRoot
        Write-Host "Successfully applied tweak to profile: $sid" -ForegroundColor Green
        $successCount++
    } catch {
        Write-Warning "Failed to apply to profile $sid : $_"
...
if (Test-Path $defaultNtUserDat) {
    ...
    if ($LASTEXITCODE -eq 0) {
        try {
            Set-MinAnimateValue -HiveRoot "HKEY_USERS\$defaultTempHive"
...
Write-Host "Sign out and back in (or restart) to ensure animations update everywhere." -ForegroundColor White -BackgroundColor DarkGreen
